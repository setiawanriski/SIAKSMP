...
if(isset($_GET['id']))
{
	$rs=mysqli_query($koneksi,"Select * from pegawai where sha1(id)='".$_GET['id']."'");
...
if(isset($_GET['edit']))
{
	...
	$rs=mysqli_query($koneksi,"Update pegawai SET namapegawai='".$_POST['namapegawai']."',pengertian='".$_POST['pengertian']."' Where sha1(id)='".$_GET['id']."'");
	if($rs)
	{
		echo "<script>window.location='?page=pegawaitampil'</script>";
	}
