...
if ( $_SESSION[username] =='id_admin'){ $tampil = mysqli_query($koneksi,"SELECT 
  * FROM data_siswa ORDER BY id_siswa DESC"); } else{ $tampil=mysqli_query($koneksi,"SELECT * 
  FROM data_siswa WHERE id_admin='$_SESSION[username]' ORDER BY id_siswa DESC"); } while($r=mysqli_fetch_array($tampil)){ 
  echo "<tr>
        <tr> 
...
<?php
   ...
   echo "<tr><th>no<th>nama mata pelajaran siswa</th><th>nilai</th><th>keterangan nilai</th><tr>";
    ...
    if ( $_SESSION[username] =='id_guru'){
      $tampil = mysqli_query($koneksi,"SELECT * FROM tbl_nilai ORDER BY id_nilai DESC");
    }
    else{
      $tampil=mysqli_query($koneksi,"SELECT * FROM tbl_nilai
