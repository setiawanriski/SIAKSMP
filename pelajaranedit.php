...
{
	$rs=mysqli_query($koneksi,"Select * from setup_pelajaran where sha1(id_pelajaran)='".$_GET['id_pelajaran']."'");
...
if(isset($_GET['edit']))
{
	...
	$rs=mysqli_query($koneksi,"Update setup_pelajaran SET nama_pelajaran='".$_POST['nama_pelajaran']."',kkm='".$_POST['kkm']."' Where sha1(id_pelajaran)='".$_GET['id_pelajaran']."'");
	if($rs)
	{
		echo "<script>window.location='?page=pelajarantampil'</script>";
	}
